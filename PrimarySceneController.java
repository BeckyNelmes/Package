...
{    
    ...
    {            
        ...
        List<Doughnuts> targetList = mainListView.getItems();  // Grab a reference to the listView's current item list.
    ...
    @FXML   void goClicked()
    {
        ...
        Iterator<Thing> filter = mainListView.getItems().iterator();
        while(filter.hasNext())
        {
            String x = searchTextField.getText();
                        
            Thing t = filter.next();
            if (!t.name.contains(x)) filter.remove();
        }
    }
